...
const getClass = async (req, res) => {
  try {
    ...
    const classList = await classes.find({ batch: batchId }, { className: 1, _id: 1 });

    // If no classes found, send a 404 error
    if (classList.length === 0) {
      return res.status(404).json({ message: "No classes found for the given batch." });
    }
...
const addClass = async (req, res) => {
  try {
    ...
    if (!batchId || !className) {
      return res.status(400).json({ message: "Batch ID and class name are required." });
    }
...
const deleteClass = async (req, res) => {
  try {
    ...
    // Return the updated list of classes
    res.status(200).json({
      classes: updatedClasses.map((classItem) => ({
        _id: classItem._id,
        className: classItem.className,
      })),
    });
...
const activateToday = async (req, res) => {
  try {
    ...
    if (!Array.isArray(classIds) || !Array.isArray(batchIds)) {
      return res.status(400).json({ message: "classIds and batchIds should be arrays." });
    }
